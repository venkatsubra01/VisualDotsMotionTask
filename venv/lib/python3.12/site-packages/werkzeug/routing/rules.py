...
class RulePart:
    ...
    either the raw content if *templates* or a regex string to match
    ...
    def _parse_rule(self, rule: str) -> t.Iterable[RulePart]:
        ...
        while pos < len(rule):
            ...
            if data["templates"] is not None:
                static_weights.append((len(static_weights), -len(data["templates"])))
                self._trace.append((False, data["templates"]))
                content += data["templates"] if static else re.escape(data["templates"])
